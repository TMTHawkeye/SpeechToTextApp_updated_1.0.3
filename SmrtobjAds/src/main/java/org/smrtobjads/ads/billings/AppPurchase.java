...
public class AppPurchase {
   ...
   /* public void initBilling(final Application application, List<PurchaseItem> listINAPId, List<PurchaseItem> listSubsId) {

        if (AppUtil.VARIANT_DEV) {
            // auto add purchase test when dev
            PurchaseItem purchaseItem;
            purchaseItem = new PurchaseItem("android.test.purchased", "", 1);
            listINAPId.add(purchaseItem);
...
//    }
 ...
 /*   private ArrayList<QueryProductDetailsParams.Product> listIdToListProduct(List<PurchaseItem> listId, String styleBilling) {
        ArrayList<QueryProductDetailsParams.Product> listProduct = new ArrayList<QueryProductDetailsParams.Product>();
        for (PurchaseItem id : listId) {
            QueryProductDetailsParams.Product product = QueryProductDetailsParams.Product.newBuilder()
                    .setProductId(id.getItemId())
                    .setProductType(styleBilling)
                    .build();
            listProduct.add(product);
        }
