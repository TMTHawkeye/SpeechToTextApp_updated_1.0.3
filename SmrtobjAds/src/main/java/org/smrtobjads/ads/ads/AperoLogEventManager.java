...
import org.smrtobjads.ads.FirebaseAnalyticsUtil;
...
public class AperoLogEventManager {
    ...
    public static void logPaidAdImpression(Context context, AdValue adValue, String adUnitId, String mediationAdapterClassName, AdType adType) {
        logEventWithAds(context, (float) adValue.getValueMicros(), adValue.getPrecisionType(), adUnitId, mediationAdapterClassName, SmartAdsConfig.PROVIDER_ADMOB);
    }

    public static void logPaidAdImpression(Context context, Object adValue, AdType adType) {
...
//        AperoAdjust.pushTrackEventApplovin(adValue, context);
    ...
    private static void logEventWithAds(Context context, float revenue, int precision, String adUnitId, String network, int mediationProvider) {
        ...
        DataStoreUtils.updateCurrentTotalRevenueAd(context, (float) revenue);
        ...
        DataStoreUtils.updateCurrentTotalRevenue001Ad(context, AppUtil.currentTotalRevenue001Ad);
    ...
    public static void logCurrentTotalRevenueAd(Context context, String eventName) {
        float currentTotalRevenue = DataStoreUtils.getCurrentTotalRevenueAd(context);
        Bundle bundle = new Bundle();
    ...
    public static void logTotalRevenue001Ad(Context context) {
        ...
        if (revenue / 1000000 >= 0.01) {
            ...
            DataStoreUtils.updateCurrentTotalRevenue001Ad(context, 0);
            Bundle bundle = new Bundle();
    ...
    public static void logTotalRevenueAdIn3DaysIfNeed(Context context) {
        long installTime = DataStoreUtils.getInstallTime(context);
        if (!DataStoreUtils.isPushRevenue3Day(context)
            ...
            DataStoreUtils.setPushedRevenue3Day(context);
        }
    }

    public static void logTotalRevenueAdIn7DaysIfNeed(Context context) {
        long installTime = DataStoreUtils.getInstallTime(context);
        if (!DataStoreUtils.isPushRevenue7Day(context)
            ...
            DataStoreUtils.setPushedRevenue7Day(context);
        }
    }
